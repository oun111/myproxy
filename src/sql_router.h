...
protected:
  ...
  using ruleHandler = int (sql_router::*)(SHARDING_VALUE*,std::vector<uint8_t>&);
...
protected:
  ...
  /* rule handlers */
  int ha_rule_rangeMap(SHARDING_VALUE*,std::vector<uint8_t>&);
  int ha_rule_modN(SHARDING_VALUE*,std::vector<uint8_t>&);
  int ha_rule_dummy(SHARDING_VALUE*,std::vector<uint8_t>&);

  int calc_intersection(std::vector<uint8_t>&,std::vector<uint8_t>&);
